...
关键设计:
  - 窗口 100 步 (5s), stride=1 密集采样
  - 物理锚点归一化: y_meas 用工作空间边界 [2.5m, 2.5m, π], u_cmd 用物理上限 [0.3, 1.76]
  - 8 通道: [y_meas(3) + innov_anchored(3) + u_cmd(2)]
...
class RobustNormalizer:
    ...
    def fit(self, X):
        ...
        self.feat_median = np.median(X[:, :, 3:6].reshape(-1, 3), axis=0).astype(np.float32)
        return self
    ...
    def save(self, filepath):
        np.savez(filepath, ymeas_median=self.ymeas_median, ymeas_scale=self.ymeas_scale,
                 feat_median=self.feat_median, feat_scale=self.feat_scale, cmd_max=self.cmd_max)


def numpy_kinematic_rollout(y0, u_seq, Ts=0.05, alpha=0.17):
    """从 y0 沿 u_seq 做欧拉积分运动学递推"""
    W = len(u_seq)
    ...
    for k in range(W - 1):
        ...
        y = y + Ts * np.array([v * cos_t - alpha * w * sin_t,
                                v * sin_t + alpha * w * cos_t, w], dtype=np.float32)
        y_kin[k + 1] = y
    return y_kin


def build_windows(data, window_size, stride):
    """从单个仿真数据提取滑动窗口 → (X(8ch), y_clean(3ch))"""
    ...
    X_w = np.zeros((n_windows, window_size, 8), dtype=np.float32)
    y_clean_w = np.zeros((n_windows, window_size, 3), dtype=np.float32)

    for i in range(n_windows):
        s, e = i * stride, i * stride + window_size
        X_w[i, :, 0:3] = y_meas_all[s:e]
        X_w[i, :, 6:8] = u_cmd_all[s:e]
        y_kin = numpy_kinematic_rollout(y_meas_all[s], u_cmd_all[s:e])
        innov = y_meas_all[s:e] - y_kin
